 ...
 */
import { APP_VERSION, APP_UPDATED_AT } from "./version.js?v=20260601-38";
import { okulAyarlariGetir, okulDonemiEtiketi } from "./school-settings.js?v=20260601-38";
import { escapeHtml } from "./utils.js?v=20260601-38";
...
function yukleTopbar() {
    ...
    <nav class="app-header navbar navbar-expand bg-body">
      <div class="container-fluid">
        <a href="dashboard.html" class="navbar-brand d-flex align-items-center gap-2">
          <span class="brand-logo-mark"><img src="icon-192.png?v=20260601-38" alt="Öğrenci Bilgileri"></span>
  ...
  document.getElementById("cikis-btn")?.addEventListener("click", async (e) => {
    e.preventDefault();
    const { logout } = await import("./auth.js?v=20260601-38");
    logout();
  });
...
async function globalAramaYukle() {
  ...
  const { tumOgrencileriGetir } = await import("./students.js?v=20260601-38");
...
async function baglantiDurumuBaslat() {
  ...
  try {
    const { db } = await import("./firebase-config.js?v=20260601-38");
